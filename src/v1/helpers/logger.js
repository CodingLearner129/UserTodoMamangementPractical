...
const fileRotateTransport = new winston.transports.DailyRotateFile({
    ...
    maxFiles: config.max_logger_files_expiration,
});
...
const logger = winston.createLogger({
    ...
    format: winston.format.combine(
        ...
        winston.format.printf(({ timestamp, level, message, stack, response, req }) => {
            let logMessage = ``;
...
export const logMessage = (error, req = {}, level = "error") => {
    ...
    logger.level = level;
    logger.log(level, error?.message ?? error, { stack: error?.stack ?? '', response: error?.response ?? {}, req: req ?? {} });
};